...
def relpath(origin, dest, altOs=None):
    """
    ...
    try:
        ...
        origin = origin.strip()
        dest = dest.strip()
        origin = osToUse.path.abspath(origin)   #.replace('\\', '/')
        _dest = osToUse.path.abspath(dest)   #.replace('\\', '/')
#        dest = osToUse.path.normpath(join(osToUse.getcwd(), dest))
        orig_list = splitall(osToUse.path.normcase(origin))
        # Don't normcase dest!  We want to preserve the case.
        dest_list = splitall(_dest)
        #
        if orig_list[0] != osToUse.path.normcase(dest_list[0]):
        ...
        for start_seg, dest_seg in zip(orig_list, dest_list):
            if start_seg != osToUse.path.normcase(dest_seg):
                break
            i += 1
        #
        ...
        segments = [osToUse.pardir] * (len(orig_list) - i)
        # Need to add the diverging part of dest_list.
        segments += dest_list[i:]
        if len(segments) == 0:
            # If they happen to be identical, use os.curdir.
            return osToUse.curdir
        else:
            return osToUse.path.join(*segments).replace('\\', '/')
    except:
        r = ""
        if altOs is not None:
            r = "remote "
        print "relpath failed using %(R)sos"%{"R":r}

def splitall(loc, altOs=None):
    """
    ...
    try:
        ...
        if altOs is not None:
            osToUse = altOs
        while loc != osToUse.curdir and loc != osToUse.pardir:
            prev = loc
            loc, child = osToUse.path.split(prev)
            if loc == prev:
                break
            parts.append(child)
        parts.append(loc)
        ...
        if altOs is not None:
            r = "remote "
        print "splitall failed using %(R)sos"%{"R":r}
...
def walkfiles(thisdir, ignoreDirs=[], hiddenDirs=True, altOs=None, ordered=False):
    """
    ...
    try:
        ...
        if altOs is not None:
            osToUse = altOs
        dirs = osToUse.listdir(thisdir)
        if ordered is True:
            dirs.sort()
        for child in dirs:
            thischild = osToUse.path.join(thisdir, child)
            if osToUse.path.isfile(thischild):
                yield thischild
            elif osToUse.path.isdir(thischild) and child not in ignoreDirs and not child.startswith("."):
                for f in walkfiles(thischild, ignoreDirs=ignoreDirs, hiddenDirs=hiddenDirs, altOs=altOs, ordered=ordered):
                    yield f
    except:
        r = ""
        if altOs is not None:
            r = "remote "
        print "walkfiles failed using %(R)sos"%{"R":r}

def walkdirs(thisdir, altOs=None):
    """
    ...
    try:
        ...
        if altOs is not None:
            osToUse = altOs
        for child in osToUse.path.listdir(thisdir):
            thischild = osToUse.path.join(thisdir, child)
            if osToUse.path.isfile(thischild):
                continue
            elif osToUse.path.isdir(thischild):
        ...
        if altOs is not None:
            r = "remote "
        print "walkdirs failed using %(R)sos"%{"R":r}

def walkemptydirs(thisdir, altOs=None):
    """
    ...
    try:
        ...
        if altOs is not None:
            osToUse = altOs
        if not osToUse.path.listdir(thisdir):
            # if the directory is empty.. then yield it
            yield thisdir   
        for child in osToUse.path.listdir(thisdir):
            thischild = osToUse.path.join(thisdir, child)
            if osToUse.path.isdir(thischild):
                for emptydir in walkemptydirs(thischild, altOs=altOs):
                    yield emptydir
    except:
        r = ""
        if altOs is not None:
            r = "remote "
        print "walkemptydirs failed using %(R)sos"%{"R":r}
...
def fullcopy(src, dst, conn=None):
    """
    ...
    try:
        if conn is None:
            ...
            osToUse = conn.root().os
        else:
            conn.execute("""import shutil""")
            _shutil = conn.modules().shutil
            osToUse = os
        if not osToUse.path.isdir(osToUse.path.dirname(dst)):
            os.makedirs(osToUse.path.dirname(dst))
        _shutil.copy(src, dst)
    except:
        if conn is None:
            r = "local system"
        else:
            r = "remote connection: %(C)s"%{"C":conn}
        print "Failed to fullcopy using %(R)s."%{"R":r}
        raise
